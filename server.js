...
function loadDatabase() {
  try {
    return yaml.load(fs.readFileSync('database.yml', 'utf8'));
  } catch (error) {
    console.log(error);
  }
}
...
function saveDatabase() {
  try {
    fs.writeFileSync('database.yml', yaml.dump(database));
  } catch (error) {
    console.log(error);
  }
}
...
function createComment(url, request) {
  ...
  if (requestComment && requestComment.articleId && database.articles[requestComment.articleId] &&
      requestComment.username && database.users[requestComment.username] && requestComment.body) {
    const comment = {
...
function createArticle(url, request) {
  ...
  if (requestArticle && requestArticle.title && requestArticle.url &&
      requestArticle.username && database.users[requestArticle.username] && validateURL(requestArticle.url)) {
    const article = {
